...
def extract_features(y, sr):
    ...
    # MFCCs (from log-mel-ish)
    mfcc = librosa.feature.mfcc(S=log_mel, n_mfcc=13)

    # Summaries over time (mean + std)
    feat = np.concatenate([
        log_mel.mean(axis=1), log_mel.std(axis=1),
        mfcc.mean(axis=1), mfcc.std(axis=1),
    ])

    # Extra: low-frequency energy ratio (AC hum/rumble often boosts this)
...
def main(args):
    ...
    print(classification_report(y_test, y_pred, target_names=["AC_OFF", "AC_ON"]))
...
if __name__ == "__main__":
    ...
    p.add_argument("--out", default="ac_detector.joblib")
