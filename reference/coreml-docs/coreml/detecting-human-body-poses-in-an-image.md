...
The sample app then proceeds to pass the input to the PoseNet’s [prediction(from:)](https://developer.apple.com/documentation/CoreML/MLModel/prediction(from:)-9y2aa) function to obtain its outputs, which the app uses to detect poses.
