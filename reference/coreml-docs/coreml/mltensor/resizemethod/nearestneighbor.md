# MLTensor.ResizeMethod.nearestNeighbor

**Case**

**Framework:** Core ML

**Availability:** iOS 18.0+, iPadOS 18.0+, Mac Catalyst 18.0+, macOS 15.0+, tvOS 18.0+, visionOS 2.0+, watchOS 11.0+

[Technologies](https://developer.apple.com/documentation/technologies) > [Core ML](../../../coreml.md) > [MLTensor](../../mltensor.md) > [MLTensor.ResizeMethod](../resizemethod.md)

---

The nearest interpolation mode where values are interpolated using the closest neighbor pixel.

## Declaration

**Platforms:** iOS, iPadOS, Mac Catalyst, macOS, tvOS, visionOS, watchOS

```objc
case nearestNeighbor
```

## See Also

### Resize methods

- [MLTensor.ResizeMethod.bilinear(alignCorners:)](bilinear(aligncorners:).md)
  The bilinear interpolation mode where values are computed using bilinear interpolation of 4 neighboring pixels.

---

*Copyright &copy; 2025 Apple Inc. All rights reserved.*
