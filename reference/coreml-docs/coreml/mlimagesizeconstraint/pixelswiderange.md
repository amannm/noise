# pixelsWideRange

**Instance Property**

**Framework:** Core ML

**Availability:** iOS 12.0+, iPadOS 12.0+, Mac Catalyst 13.1+, macOS 10.14+, tvOS 12.0+, visionOS 1.0+, watchOS 5.0+

[Technologies](https://developer.apple.com/documentation/technologies) > [Core ML](../../coreml.md) > [MLImageSizeConstraint](../mlimagesizeconstraint.md)

---

The range of widths a model’s image feature accepts as input or produces as output.

## Declaration

**Platforms:** iOS, iPadOS, Mac Catalyst, macOS, tvOS, visionOS, watchOS

```objc
var pixelsWideRange: NSRange { get }
```

## See Also

### Accessing the image size ranges

- [pixelsHighRange](pixelshighrange.md)
  The range of heights a model’s image feature accepts as input or produces as output.

---

*Copyright &copy; 2025 Apple Inc. All rights reserved.*
